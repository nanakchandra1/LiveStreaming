...
open class TagView: UIButton {
    ...
    @IBInspectable open var cornerRadius: CGFloat = 0 {
        didSet {
            ...
            layer.masksToBounds = false//cornerRadius > 0
        }
    }
    ...
    private func setupView() {
        ...
        self.blackBackView.backgroundColor = UIColor.black
        addSubview(removeButton)
        self.blackBackView.frame = CGRect(x: self.removeButton.frame.origin.x + 15 , y: self.removeButton.frame.origin.y - 7 , width: 15, height: 15)
        self.blackBackView.layer.cornerRadius =  self.blackBackView.frame.size.height / 2
        self.blackBackView.clipsToBounds = false
    self.layer.masksToBounds = false
        self.removeButton.layer.masksToBounds = false
       // addSubview(blackBackView)
        self.removeButton.addSubview(blackBackView)
        self.blackBackView.bringSubview(toFront:  self.removeButton)
        self.crossImageView.image = UIImage(named: "ic_live_cut")
        self.crossImageView.frame = CGRect(x: self.blackBackView.frame.origin.x - 15 , y: self.removeButton.frame.origin.y , width: 15, height: 15)
        self.blackBackView.addSubview(self.crossImageView)
        
        removeButton.tagView = self
        self.removeButton.backgroundColor = UIColor.clear
        self.removeButton.layer.cornerRadius = self.removeButton.frame.size.height / 2
         self.removeButton.clipsToBounds = false
